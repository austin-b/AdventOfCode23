...
fn main() {
    ...
    for b in blocks {
        if b.contains("seeds") {
            println!("b: {:?}", b);
            let mut seed_nums: VecDeque<u64> = b.split(": ").collect::<Vec<&str>>()[1].split_whitespace().into_iter().map(|x| x.trim().parse().unwrap()).collect();

            loop {
                let start = seed_nums.pop_front();
                let length = seed_nums.pop_front();
                if start.is_none() || length.is_none() { break; }

                seed_ranges.push(RangeInclusive::new(start.unwrap(), start.unwrap() + length.unwrap()));
            }
